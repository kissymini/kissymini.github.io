...
//            });
        ...
        it('404时，jsonp不触发回调', function (done) {
            var ret404;
            IO({
                url: '../data/404',
                dataType: 'jsonp',
...
//            });
        ...
        it('404时，cross domain jsonp不触发回调', function (done) {
            var ret404;
            IO({
                url: '../data/404',
                dataType: 'jsonp',
...
//            });
    ...
    describe('jsonp', function () {
        ...
        it('自定义callback', function (done) {
            var ok = false;
//            runs(function () {
                window['customCallback'] = function (data) {
                    ok = true;
                    if(typeof data == "object" && data.callback == "customCallback") {
                        done()
                    }else {
                        done('error data');
                    }
...
//            });
        ...
        it('自定义callbackName', function (done) {
            var ok = false;
//            runs(function () {
                IO({
                    url: '../data/interface.jss?t=get',
...
//            });
        ...
        it('不带参数请求jsonp', function (done) {

            var ok = 0;

//            runs(function () {
                KISSY.jsonp('../data/interface.jss', function (data) {
                    ok = true;
//                    expect(typeof data).toBe('object');
...
//            });
        ...
        it('带参数提交jsonp，获取回调参数', function (done) {
            var ok = 0;
            KISSY.jsonp('../data/interface.jss?sleep=0', {
                my_param: 'taobao'
            }, function (data) {
//                expect(typeof data).toBe('object');
...
//            });
        ...
        it("can ignore protocol", function (done) {
            var ok = 0;
            KISSY.jsonp('//' + location.hostname + ':'+SERVER_CONFIG.ports[1]+'/kissy/src/io/tests/data/interface.jss', {
                my_param: 'taobao'
            }, function (data) {
//                expect(typeof data).toBe('object');
//                expect(data).not.toBe(undefined);
...
//            });
    ...
    describe('同步请求的test case', function () {
        ...
        it('get 同步加载,等待2秒钟继续执行', function (done) {
            var d = 0;
            IO({
                type: 'get',
...
//            expect(d['name']).toBe('test');
        ...
        it('post 同步加载,等待2秒钟继续执行', function (done) {
//            runs(function () {
                var d = 0;
                IO({
                    type: 'post',
...
//            });
    ...
    describe('post', function () {
        ...
        it('能正确发起post 请求，无参数,正确获取回调参数，类型是text/json', function (done) {

            var ok = false;
            KISSY.post('../data/interface.jss?t=post&contentType=text/json', function (data) {
                ok = true;
//                expect(typeof data).toBe('object');
//                expect(data).not.toBe(undefined);
...
//            });
        ...
        it('能正确发起 post 请求，无参数,正确获取回调参数', function (done) {

            var ok = false;
            KISSY.post('../data/interface.jss?t=post', function (data, status, xhr) {
                ok = true;
...
//            });
        ...
        it('能正确发起 post 请求, 参数为json，并正确获取参数', function () {
            ...
            var ok = false;
            KISSY.post('../data/interface.jss?t=post', {
                type: 'post',
                name: 'test',
        ...
        it('正确处理 dataType 为 json 的情况', function () {
            ...
            runs(function () {
                KISSY.post('../data/interface.jss?contentType=text/json',
                    function (data) {
                        ok = true;
                        o = data;
                    }, 'json');
        ...
        it('json 参数 可以覆盖url参数', function () {
            var ok = false;
            KISSY.post('../data/interface.jss?foo=sk1', {
                foo: 'sk2'
            }, function (data) {
                ok = true;
                expect(typeof data).toBe('object');
        ...
        it('正确处理 dataType 为 jsonp 的情况', function () {
            ...
            runs(function () {
                ...
                KISSY.post('../data/interface.jss', function (data) {
                    ok = true;
                    o = data;
                }, 'jsonp');
            });
        ...
        it('正确处理 dataType 为 script 的情况', function () {
            ...
            runs(function () {
                // 这里和get的处理一致
                KISSY.post('../data/interface.jss?type=post&dataType=script',
                    function (data) {
                        ok = true;
        ...
        it('正确处理 dataType 为 script 的情况，并且 contentType 设置正确', function () {
            ...
            runs(function () {
                // 这里和get的处理一致
                KISSY.post('../data/interface.jss?type=post' +
                    '&dataType=script' +
        ...
        it('正确处理 dataType 为 xml 的情况,回调参数为 xml 对象', function () {
            ...
            runs(function () {
                ...
                //这里'xml'的参数可以省略
                KISSY.post('../data/xml.jss', function (data) {
                    ok = true;
                    o = data;
                }, 'xml');
            });
        ...
        it('正确处理 dataType 为 xml 的情况,回调参数为json对象', function () {
            ...
            runs(function () {
                ...
                KISSY.post('../data/xml-json.jss', function (data) {
                    ok = true;
                    o = data;
                }, 'json');
            });
        ...
        it('xhr 方式时，能正确设置 callback 里的 this', function () {
            ...
            KISSY.post('../data/interface.jss', function () {
                ok = true;
                // this === config
        ...
        it('getScript 方式时，能正确设置 callback 里的 this', function () {
            ...
            KISSY.post('../data/interface.jss?t=get&dataType=script', function () {
                ok = true;
                expect(this.type).toBe('POST');
            }, 'script');
        ...
        it('能正确获取返回值', function () {
            var xhr = KISSY.post('../data/interface.jss');
            expect('abort' in xhr).toBe(true);

            // 注：jQuery 里，不跨域时，jsonp 返回 xhr. 跨域时，返回 void
            xhr = KISSY.jsonp('../data/interface.jss');
            expect('abort' in xhr).toBe(true);
        });
    ...
    describe('get', function () {
        ...
        it('能正确处理 dataType 为 html 和 text 的情况', function () {
            ...
            runs(function () {
                ok = false;
                KISSY.get('test.jss', function (data) {
                    ok = true;
                    o = data;
                });
            });
        ...
        it('能正确获取 callback 参数', function () {
            ...
            KISSY.get('../data/interface.jss?t=get', function (data, textStatus, xhr) {
                ok = true;
        ...
        it('能正确处理 data 参数', function () {
            ...
            KISSY.get('../data/interface.jss?t=get', {'data': 'hello'}, function (data) {
                ok = true;
                var o = S.parseJson(data);
        ...
        it('能正确处理 dataType 为 json 的情况', function () {
            ...
            runs(function () {
                //KISSY.get('http://test.com/kissy/src/io/tests/interface.jss?t=get', function(data) {
                // test.com -> 127.0.0.1
                // jQuery 里，当跨域时，dataType 为 json 时，依旧会调用 xhr. KISSY 处理逻辑与 jQuery 一致。
                KISSY.get('../data/interface.jss?t=get', function (data) {
                    ok = true;
                    o = data;
                }, 'json');
            });
        ...
        it('能正确处理 dataType 为 script 的情况', function () {
            ...
            runs(function () {
                ...
                KISSY.get('../data/interface.jss?t=get&dataType=script', function (d) {
                    ok = true;
        ...
        it('能正确处理 dataType 为 xml 的情况', function () {
            ...
            runs(function () {
                ...
                KISSY.get('../data/xml.jss', function (data) {
                    ok = true;
                    o = data;
                });
            });
        ...
        it('xhr 方式时，能正确设置 callback 里的 this', function () {
            ...
            KISSY.get('../data/interface.jss?t=get', function () {
                ok = true;
        ...
        it('getScript 方式时，能正确设置 callback 里的 this', function () {
            ...
            KISSY.get('../data/interface.jss?t=get&dataType=script', function () {
                ok = true;
                expect(this.type).toBe('GET');
            }, 'script');
            ...
            runs(function () {
                S.each(['start', 'send', 'success', 'complete'], function (type) {
                    IO.on(type, function (ev) {
                        called.push(ev.type);
                        expect(ev.io.cfg).not.toBe(undefined);
                        expect(ev.type).toBe(type);
                    });
                });
                KISSY.get('../data/interface.jss?t=' + S.now(), function () {
                    ok = true;
                });
            });
